...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    @commands.command(name="queue_playlist")
    async def queue_playlist(self, ctx, user: t.Optional[discord.Member]):
      ...
      if str(member.id) in data:
        playlist_url = data[str(member.id)]['playlist_link']
        
        #await ctx.send(f"Playlist Url: {playlist_url} \n Work in this command is still going on, so yea wait some time xD")

      else:
        await ctx.send(f"{member.mention} Don't Have any playlist saved")
